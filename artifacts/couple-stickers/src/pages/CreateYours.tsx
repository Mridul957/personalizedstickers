    ...
    <div className="min-h-screen pb-44" style={{ background:"hsl(204,46%,9%)" }}>
      ...
      <div className="max-w-6xl mx-auto px-4 pt-10 pb-6">
        ...
        <motion.div initial={{ opacity:0, y:24 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.6 }}
          ...
          {/* Encouragement / complete banner */}
          <AnimatePresence>
            {atMin && !atMax && (
              <motion.div key="encourage"
                initial={{ opacity:0, y:-8, scale:0.95 }} animate={{ opacity:1, y:0, scale:1 }} exit={{ opacity:0, y:-8 }}
                className="mt-4 inline-flex items-center gap-2 px-5 py-2 rounded-full text-sm font-semibold"
                style={{ background:"rgba(43,170,143,0.12)", border:"1px solid rgba(43,170,143,0.28)", color:"rgba(43,170,143,0.9)", boxShadow:"0 0 20px rgba(43,170,143,0.15)" }}>
            ...
            {atMax && (
              <motion.div key="complete"
                initial={{ opacity:0, y:-8, scale:0.95 }} animate={{ opacity:1, y:0, scale:1 }} exit={{ opacity:0, y:-8 }}
                className="mt-4 inline-flex items-center gap-2 px-5 py-2 rounded-full text-sm font-semibold"
                style={{ background:"linear-gradient(135deg, rgba(43,170,143,0.2), rgba(232,196,90,0.15))", border:"1px solid rgba(232,196,90,0.35)", color:"rgba(232,196,90,0.9)", boxShadow:"0 0 24px rgba(232,196,90,0.15)" }}>
        ...
        {visible.length === 0 && (
          <div className="text-center py-24">
            ...
            <p className="text-sm" style={{ color:"rgba(43,170,143,0.4)" }}>No stickers available yet. Ask an admin to add some!</p>
          </div>
        )}
            ...
            return (
              <motion.div key={sticker.id}
                ...
                <div className="aspect-square flex items-center justify-center relative"
                  style={{ background:isSelected ? "rgba(43,170,143,0.08)" : "rgba(29,58,74,0.35)", margin:"10px 10px 0 10px", borderRadius:"14px" }}>
                  {isSelected && (
                  ...
                  {/* Trending badge */}
                  {sticker.trending && (
                    <div className="absolute top-1.5 left-1.5 text-[10px] font-bold px-1.5 py-0.5 rounded-full"
                      style={{ background:"rgba(251,113,183,0.2)", color:"rgba(251,113,183,0.9)", border:"1px solid rgba(251,113,183,0.3)" }}>
                      🔥
                    </div>
                  )}
                </div>
          ...
          <motion.div
            ...
            <div className="max-w-2xl mx-auto rounded-2xl px-5 py-4"
              ...
              <div className="flex items-center gap-3">
                ...
                {/* CTA */}
                <motion.button
                  onClick={() => atMin && setShowSuccess(true)}
      ...
      <AnimatePresence>
        {showSuccess && (
          <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
            ...
            <motion.div
              initial={{ scale:0.8, y:32 }} animate={{ scale:1, y:0 }} exit={{ scale:0.8, y:32 }}
              ...
              <motion.div className="text-5xl mb-4"
                animate={{ rotate:[0,15,-12,8,-5,0], scale:[1,1.25,1] }} transition={{ duration:0.9 }}>
                🎉
              </motion.div>
